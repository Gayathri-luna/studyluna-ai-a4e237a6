...
/**
 * Small quick-access entry point to the existing StudyLuna.ai assistant,
...
export function AskLunaButton({ topic, variant = "floating", label, className }: Props) {
  const text = label ?? `Ask StudyLuna.ai about ${topic}`;
  ...
  return (
    ...
    >
      ...
      <span className="hidden text-xs font-semibold sm:inline">Ask StudyLuna.ai</span>
    </Link>
  );
}
